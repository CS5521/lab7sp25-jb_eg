...
scheduler(void)
{
  ...
  for(;;){
    ...
    // this iw=s total tickets
    int tt = 0;
    // this loops and see what all the running processes to give a ticket to
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      if(p->state == RUNNABLE) {
        tt += p->tickets;
      }
    ...
    // if there are no tickets release lock and go back through loop
    if(tt == 0) {
      release(&ptable.lock);
      continue;
    }

    // pick a number for the lottery 
    int winner = random() % tt;
    int numProcesses = 0;
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      numProcesses += p->tickets;
      if(numProcesses > winner) {
        c->proc = p;
        switchuvm(p);
        p->state = RUNNING;
        p->ticks++;
